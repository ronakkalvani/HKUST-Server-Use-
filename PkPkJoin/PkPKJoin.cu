...
int main() {
    ...
    Splitterss<<<numBlocks, BLOCK_THREADS>>>(d_splitters, d_samples, sample_size, p);
    (cudaGetLastError());
    (cudaDeviceSynchronize());
    ...
    (cudaMalloc(&d_partition_counts, p * sizeof(int)));
    (cudaMalloc(&d_partition_starts, p * sizeof(int)));
    (cudaMalloc(&d_partition_offsets, p * sizeof(int)));

    (cudaMemset(d_partition_counts, 0, p * sizeof(int)));
    (cudaMemset(d_partition_starts, 0, p * sizeof(int)));
    (cudaMemset(d_partition_offsets, 0, p * sizeof(int)));
