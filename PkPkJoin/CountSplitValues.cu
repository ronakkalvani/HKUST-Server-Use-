...
__global__ void countSplits(int* split_indices, int* split_counts, int num_elements, int num_splits) {
    ...
    for (int i = tid; i < num_splits; i += block_size) {
        atomicAdd(&split_counts[i], shared_counts[i]);
    }
}
...
int main() {
    ...
    const int num_splits = 100; // Assume there are 100 splits, adjust as needed
    const int num_blocks = (num_elements + block_size - 1) / block_size;

    int* h_split_indices = new int[num_elements];
    int* h_split_counts = new int[num_splits]();
    ...
    cudaMalloc(&d_split_counts, num_splits * sizeof(int));
    cudaMemcpy(d_split_indices, h_split_indices, num_elements * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemset(d_split_counts, 0, num_splits * sizeof(int));

    countSplits<<<num_blocks, block_size, num_splits * sizeof(int)>>>(d_split_indices, d_split_counts, num_elements, num_splits);

    cudaMemcpy(h_split_counts, d_split_counts, num_splits * sizeof(int), cudaMemcpyDeviceToHost);

    for (int s = 0; s < num_splits; ++s) {
        std::cout << "Split " << s << " counts: " << h_split_counts[s] << "\n";
    }
