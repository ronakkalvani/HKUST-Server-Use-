...
{
    typedef cub::BlockLoad<int, BLOCK_THREAD, ITEMS_PER_THREAD, cub::BLOCK_LOAD_WARP_TRANSPOSE> BlockLoadT;
    typedef cub::BlockStore<int, BLOCK_THREAD, ITEMS_PER_THREAD, cub::BLOCK_STORE_WARP_TRANSPOSE> BlockStoreT;
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; i++) {
        int index = block_start + threadIdx.x * ITEMS_PER_THREAD + i;
        thread_keys[i] = (index < block_end) ? d_in[index] : INT_MAX;
    }

    BlockLoadT(temp_storage.load).Load(thread_keys, thread_keys, valid_items);
