...
__global__ void BlockSortKernel2(int *d_in, int *d_out, int *block_indices, int num_blocks, int num_elements) {
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; i++) {
        int global_idx = block_start + threadIdx.x * ITEMS_PER_THREAD + i;
        if (global_idx < block_end) {
            thread_keys[i] = d_in[global_idx];
        } else {
            thread_keys[i] = INT_MAX;  // Use INT_MAX to indicate invalid data
        }
    }

    // Load data
    BlockLoadT(temp_storage.load).Load(d_in + block_start, thread_keys, block_size);
    ...
    BlockStoreT(temp_storage.store).Store(d_out + block_start, thread_keys, block_size);
}
